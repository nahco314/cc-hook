...
use nix::{
    ...
    unistd::{close, dup2, execvp, fork, ForkResult, setsid, tcsetpgrp, getpid, isatty},
};
use std::{
    ffi::CString,
    os::unix::io::{AsRawFd, FromRawFd, BorrowedFd},
};
use tokio::{
...
pub async fn run_with_hooks(
    ...
    let master_async = tokio::fs::File::from_std(unsafe {
        std::fs::File::from_raw_fd(master_dup)
    });
